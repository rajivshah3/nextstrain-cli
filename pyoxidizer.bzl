...
#
# FROM_DIST
#    Specifies a directory containing pre-built dists to use for packaging
#    Nextstrain CLI (e.g. dist/) instead of packaging from the source dir.
#
FROM_DIST = VARS.get("FROM_DIST")
...
def make_exe():
    ...
    # equivalent to `python -m nextstrain.cli …`; utf8_mode to `-X utf8` or
    # `PYTHONUTF8=1`.
    ...
    # attributes.
    if FROM_DIST:
        exe.add_python_resources(exe.pip_install(["--find-links", FROM_DIST, "nextstrain-cli"]))
    else:
